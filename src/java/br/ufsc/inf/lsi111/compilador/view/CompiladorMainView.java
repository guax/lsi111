...
public class CompiladorMainView extends javax.swing.JFrame {
	...
	private void initComponents() {
		...
		jPanel1.setBorder(javax.swing.BorderFactory
				.createTitledBorder("Código Fonte"));
		...
		SyntheticAnalysisMenu.setAccelerator(javax.swing.KeyStroke
				.getKeyStroke(java.awt.event.KeyEvent.VK_S,
						java.awt.event.InputEvent.CTRL_MASK));
		SyntheticAnalysisMenu.setText("Sintático");
		SyntheticAnalysisMenu
				.addActionListener(new java.awt.event.ActionListener() {
					public void actionPerformed(java.awt.event.ActionEvent evt) {
	...
	private void syntheticAnalysisMenuActionPerformed(
		...
		if (!sourceText.getText().isEmpty()) {
			outputText.setText("");
			try {
				String retorno = model.executaAnalisadorSintatico(sourceText
						.getText());
				outputText.setText(retorno);
			} catch (AnalysisError error) {
				exceptionHandling(error);
		...
		if (!sourceText.getText().isEmpty()) {
			...
			if (fileChooserReturn == JFileChooser.APPROVE_OPTION) {
				...
				try {
					...
					File file = addExtensionToFile(fileChooser
							.getSelectedFile());
